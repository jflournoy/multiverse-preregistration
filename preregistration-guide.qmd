...
# Other impinging constructs (auxiliary constructs)
...
# What are the potential causal models?

The variables you've listed above may play different roles in different causal models.

1. What is the causal flow between the causes and the effects? [@crashcourse]
...
# Identify different operationalizations or processing methods for measured variables
...
# Categorize the decision points revealed by your lists

Here, we will use the framework of @delgiudice to group these various decision points into three types. What we do next, depends on this categorization,
...
Using the above categories, @delgiudice suggest the following approach:
...
How will you describe the results of your multiverse analysis and interpret the results? Is your criterion of interest consistent across all Type E decisions? Is there variation across Type N and Type U decisions? Some authors have proposed methods for statistical comparisons between multiverses. Others caution against the probabilistic interpretation of multiverse analyses these methods require, opting instead to describe them as "possibilistic." Since we are uncertain about the single best analytic plan, it is possible that any single one of the results from the multiverse is the closest to the truth.
